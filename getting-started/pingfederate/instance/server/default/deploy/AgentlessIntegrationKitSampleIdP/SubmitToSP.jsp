...
try
{
	...
	String[] strArr = request.getParameter("groups").toString().split("\\s*,\\s*");
	List<String> items = Arrays.asList(strArr);
	userAttributes.put( "groups", items);
	...
	String resumePath = (String)request.getSession(true).getAttribute("resumePath");
	if(resumePath != null)
	{
		response.sendRedirect(PF_BASE_URL + ":" + PF_PRIMARY_SSL_PORT + resumePath + "?REF=" + idpReferenceValue);
	}
	else{
